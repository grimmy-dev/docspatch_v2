...
def _build_prompt(summaries: Iterable[FileSummary]) -> str:
    ...
    return (
        ...
        "- file_tiers: for every file path listed below, tag 'public' or 'internal'. "
        "'public' = a README for this project would mention the module: declared entry "
        "points, CLI commands, or the public API a user imports. 'internal' = supporting "
        "plumbing a README never documents. Judge by the public surface, not by folder "
        "name or location; the same rule holds for a CLI, a library, or a service. Key "
        "each entry by the file path exactly as shown.\n"
        f"{SUMMARY_VOICE}"
